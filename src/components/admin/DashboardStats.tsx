...
useEffect(() => {
  ...
  const totalSold = ticketTypes.reduce(
    (acc, type) =>
      acc +
      type.phases.reduce((sum, phase) => sum + phase.sold, 0),
    0
  )
  ...
  const totalRevenue = ticketTypes.reduce(
    (acc, type) =>
      acc +
      type.phases.reduce(
        (sum, phase) => sum + phase.sold * phase.price,
        0
      ),
    0
  )
  ...
  const salesByType = ticketTypes.map(type => {
    ...
    const vendidos = type.phases.reduce(
      (acc, phase) => acc + phase.sold,
      0
    )

    const receita = type.phases.reduce(
      (acc, phase) => acc + phase.sold * phase.price,
      0
    )

    const ticketMedio =
      vendidos > 0 ? Math.round(receita / vendidos) : 0
  ...
  const showsCountByDay = festivalDays.map(day => ({
    day,
    shows: store.performances.filter(
      p => Number(p.day) === day
    ).length
  }))
  ...
  const passaporteRevenue =
    revenueByType.find(t => t.name === "Passaporte 3 Dias")
      ?.receita || 0
  ...
  const revenueByDay = festivalDays.map(day => {
    ...
    const showsInDay =
      showsCountByDay.find(d => d.day === day)?.shows || 0

    const revenueSingleDay =
      singleDayRevenue * dayWeights[day]
    ...
    return {
      day: `Dia ${day}`,
      receita: Math.round(
        revenueSingleDay + revenuePassaporte
      ),
      shows: showsInDay
    }
  })
  ...
  const revenueEvolution = [
    { etapa: "Lançamento", receita: Math.round(totalRevenue * 0.20) },
